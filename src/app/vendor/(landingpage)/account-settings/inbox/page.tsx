...
const GET_VENDOR_CHATS = gql`
  query GetVendorChats($vendorId: ID!) {
    vendorChatList(vendorId: $vendorId) {
      chatId
      buyer {
        id
        name
...
const VendorInbox = () => {
  ...
  const vendorId = vendorInfo ? JSON.parse(vendorInfo).id : null;
  ...
  const [loadChats] = useLazyQuery<{ vendorChatList: ChatItem[] }>(GET_VENDOR_CHATS, {
    onCompleted: (data) => {
      console.log("Vendor chats loaded:", data.vendorChatList);
      setChatList(data.vendorChatList)
    },
  ...
  useEffect(() => {
    if (vendorId) loadChats({ variables: { vendorId } });
  }, [vendorId]);
  ...
  useEffect(() => {
    console.log("Vendor ID from cookie/session:", vendorId);
  }, [vendorId]);
  

  const handleSendMessage = async () => {
    if (!newMessage.trim() || !selectedChat || !vendorId) return;
    const receiverId = selectedChat.buyer.id;
