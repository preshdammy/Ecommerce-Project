...
const Purchases = () => {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      if (pending) {
        if (pending.scrollLeft + pending.clientWidth >= pending.scrollWidth) {
          pending.scrollLeft = 0;
        } else {
          pending.scrollLeft += scrollWidth;
        }
      }

      if (completed) {
        if (completed.scrollLeft + completed.clientWidth >= completed.scrollWidth) {
          completed.scrollLeft = 0;
        } else {
          completed.scrollLeft += scrollWidth;
        }
      }
    }, 3000);
  ...
  return ( 
    <div className="min-h-screen bg-gray-100 md:px-12 py-6 md:py-10">
      ...
      <div ref={pendingRef} className="w-full flex gap-4 overflow-x-auto whitespace-nowrap mb-5 scrollbar-hide">
        {[...Array(8)].map((_, index) => (
          <DeliveryCard key={index} />
        ))}
      </div>

      <h2 className="text-[24px] font-semibold mb-4 text-[#939090]">Completed deliveries</h2>
      <div ref={completedRef} className="w-full flex gap-4 overflow-x-auto whitespace-nowrap mb-5 scrollbar-hide">
        {[...Array(8)].map((_, index) => (
          <DeliveryCard key={index} />
        ))}
      </div>
...
const DeliveryCard = () => (
  <div className="bg-white rounded-2xl border border-[#CCE5FF] h-[40vh] w-[280px] flex-shrink-0 hover:border hover:border-blue-600">
